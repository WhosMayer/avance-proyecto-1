...
const analitica = (θ0, L, t) => θ0 * Math.cos(Math.sqrt(g / L) * t);
// ─── Métricas de error ─────────────────────────────────────
const errAbs = (v, a) => Math.abs(v - a);
const errRelP = (v, a) => Math.abs(v) < 1e-15 ? 0 : (Math.abs(v - a) / Math.abs(v)) * 100;
const errAprxP = (act, ant) => Math.abs(act) < 1e-15 ? 0 : (Math.abs(act - ant) / Math.abs(act)) * 100;
const tolCS = (c) => 0.5 * Math.pow(10, 2 - c);
// ─── Euler explícito ───────────────────────────────────────
function eulerPendulo(θ0, ω0, L, hMuestra, tTotal, n) {
    const H = 0.01, datos = [];
    let θ = θ0, ω = ω0, t = 0, θPrev = θ0;
    for (let tM = 0; tM <= tTotal + 1e-9; tM += hMuestra) {
        while (t < tM - 1e-9) {
            θ += H * ω;
            ω += H * (-(g / L) * sinTaylor(θ, n));
            t += H;
        }
...
// ─── Runge-Kutta 4 ─────────────────────────────────────────
function rk4Pendulo(θ0, ω0, L, h, tTotal) {
...
function analisisPasos(θ0, ω0, L, n) {
...
// ─── Muestreo ──────────────────────────────────────────────
function muestrear(arr, max = 300) {
    if (arr.length <= max)
        return arr;
...
const charts = {};
const ds = (data, color, extra = {}) => ({ data, borderColor: color, borderWidth: 1.8, tension: 0.2, ...extra });
const CHART_OPTS = {
...
function plot(id, labels, datasets, opts = {}) {
    if (charts[id])
        charts[id].destroy();
...
const dotStyle = (c) => ({ borderWidth: 2, pointRadius: 5, pointBackgroundColor: c, tension: 0 });
function renderGraficas(e, r) {
...
function renderTaylorGrafica() {
    const xV = parseFloat(document.getElementById('taylorX')?.value || '1.2');
    ...
    const tb = document.getElementById('tbodyTaylor');
    if (tb)
        tb.innerHTML = filas.map(({ n, aprox, error }) => {
...
function demoRedondeo() {
    ...
    const tb = document.getElementById('tbodyRedondeo');
    if (tb)
        tb.innerHTML = ejemplos.map(({ expr, res, esp }) => {
...
function llenarTablaPasos(d) {
    const tb = document.getElementById('tbodyPasos');
    if (!tb)
        return;
    tb.innerHTML = d.map(r => `<tr><td>${r.h}</td><td>${r.pasos}</td><td class="mono">${r.errMax.toExponential(4)}</td>
      <td class="mono">${r.errFinal.toExponential(4)}</td></tr>`).join('');
}
// ─── Stats resumen ─────────────────────────────────────────
function mostrarStats(d, p) {
...
function simular() {
    ...
    if (h <= 0 || h > 1) {
        alert('Usa un paso h entre 0.001 y 1.');
        return;
    }
    ...
    if (out)
        out.textContent = `✔ Simulación ejecutada — ${ed.length} pasos, h=${h}, n_Taylor=${n}`;
}
